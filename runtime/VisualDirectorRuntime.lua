...
--
-- A committed package stores its timeline as attributes and keeps native
-- effects dormant until this module drives them. It walks a package, reads
...
export type PlayOptions = {
	speed: number?,
	loop: boolean?,
	-- One-shot effects previewed repeatedly should not accumulate particles from
	-- the previous cycle. Seamless authored loops can leave this false.
	clearParticlesOnLoop: boolean?,
...
function Runtime.play(root: Instance, options: PlayOptions?): Playback
	...
	-- Start from a clean slate even for captured third-party packages whose
	-- native instances may have been enabled before Visual Director owned them.
	for _, node in ipairs(nodes) do
		node.active = true
		setActive(node, false)
	end
	...
	connection = RunService.Heartbeat:Connect(function(delta)
		...
		if elapsed >= duration then
			if settings.loop then
				-- Force a real timeline boundary. Otherwise a node spanning the end
				-- and beginning remains active and its burst never fires again.
				for _, node in ipairs(nodes) do
					local resetNode = settings.clearParticlesOnLoop == true
					if not resetNode then
						for _, emitter in ipairs(node.emitters) do
							local burst = emitter:GetAttribute("BurstCount")
							if type(burst) == "number" and burst > 0 then resetNode = true; break end
						end
					end
					if resetNode then
						setActive(node, false)
						if settings.clearParticlesOnLoop == true then
							for _, emitter in ipairs(node.emitters) do emitter:Clear() end
						end
					end
				end
